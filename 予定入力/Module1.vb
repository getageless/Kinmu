...
Module Module1
    ...
    Public WindowNameJisseki As String = "管制日報入力＜実績＞"
    Public windownameJisseki2 As String = "勤務日程入力＜実績＞"
...
Module Module2
    ...
    Public Function 勤務日程入力画面取得()
        ...
        If hWnd = 0 Then
            ...
            If hWnd = 0 Then
                ...
                KinmuHwnd = hWnd
                WindowKinmuNitteiNyuryoku = "管制日報入力＜実績＞"
                '見つからなかった場合、終了
                If hWnd = 0 Then
                    hWnd = FindWindowEx(0, 0, ClassName, windownameJisseki2)
                    KinmuHwnd = hWnd
    ...
    Public Function 勤務法定休出処理()
        ...
        If Not 既に法定休出処理 Then
            '注意！！2014年6月現在、休日タブでエラー発生中のため、タブ選択しない！！
            勤務タブ休み選択(1)
        End If
        'プロパティコンディションを設定（AutomationIDPropertyを識別用に設定する）
        pcFirst = New Windows.Automation.PropertyCondition(Windows.Automation.AutomationElement.AutomationIdProperty, "81")
    ...
    Public Function 社員番号から管制社員(ByVal 社員番号 As String)
        ...
        Try
            ...
            For Each LItem In aecListItem
                If CStr(Left(LItem.current.name, 6)) = 社員番号 Then
                    '社員番号で検索し、見つかったらリストの名前を社員番号に代入する
                    社員番号 = LItem.current.name
                    pcThird = New Windows.Automation.PropertyCondition(Windows.Automation.AutomationElement.NameProperty, 社員番号)
                    Exit For
                Else
    ...
    End Function
    Public Function 管制物件選択(ByVal リストインデックス As Integer)
